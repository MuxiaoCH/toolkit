...
TICPATH = os.path.join(DATDIR, "TICKERS.txt")
...
def get_tics(pth):
    ...
    with open(pth, 'r') as fobj:
        for line in fobj:
            exchange, tic = tuple(line.strip().replace("\n","").lower().split('='))
            dict[tic] = exchange
        return dict
...
def read_dat(tic):
    ...
    pth = f'{os.path.join(DATDIR,tic)}_prc.dat'
    result = []
    with open(pth, 'r') as fobj:
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        dict[column] = line[begin: begin+COLWIDTHS[column]]
        begin += COLWIDTHS[column]
    return dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    if tickers_lst and not all(t in tic_exchange_dic for t in tickers_lst):
        raise Exception(f"Invalid tickers: {', '.join(set(tickers_lst) - set(tic_exchange_dic.keys()))}")
    elif tickers_lst == []:
...
def verify_cols(col_lst=None):
    ...
    invalid_cols = set(col_lst) - set(COLUMNS)
    if invalid_cols:
        raise Exception(f"{invalid_cols} are not found in COLUMNS.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    return {tic: {'exchange': tic_exchange_dic[tic],
                  'data': [dict((col, line_to_dict(line)[col]) for col in col_lst)
