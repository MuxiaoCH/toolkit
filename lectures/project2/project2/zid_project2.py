...
def read_prc_csv(tic):
    ...
    csv_path = os.path.join(cfg.DATADIR, f"{modified_tic}_prc.csv")
...
def get_avg(df, col, year):
    ...
    df_new = df.loc[f"{year}", col]

    avg = df_new.mean(axis=0, skipna=True)
...
def get_ann_ret(ser, start, end):
    ...
    ser1 = ser[ser.index >= start]
    ser1 = ser1[ser1.index <= end]
    ser1 = ser1.dropna()
    ser1 += 1
    total_ret = 1
    N = 0
    for index, value in ser1.items():
        total_ret *= value
        N += 1
    return round(total_ret, 10) ** (252 / N) - 1
